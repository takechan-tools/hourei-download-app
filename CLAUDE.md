...
- `law-list-container`の**外**に配置すること（中に入れると検索欄を空にした時に一緒に非表示になる）
- 検索入力のイベントハンドラで検索欄が空になった時にも `updateSelectedLawsDisplay()` を呼び出す必要がある
- これにより、検索結果一覧が非表示でも選択済み法令は常に表示される
